# MyVideoShop 短视频带货商城系统
基于 Uniapp + Webman 开发的高性能短视频电商系统 | 全开源无加密 | 支持短视频带货 + 分销代理

## 项目介绍
MyVideoShop 是一套专注于短视频带货的电商系统，采用 Uniapp 前端 + Webman 高性能后端开发，
支持短视频种草、商品购买、分销代理、活动营销等核心功能，后台简洁易用，前端美观大方。

系统功能完整，离商用仅一步之遥，适合开发者学习、二次开发、项目落地使用。

## 功能亮点
 - ✅ **短视频带货**：视频内直接挂载商品，一键跳转购买，转化路径短
 - ✅ **完整电商模块**：商品管理、订单管理、库存管理、支付对接
 - ✅ **分销代理系统**：推广分成、下级返利、佣金提现，快速裂变获客
 - ✅ **丰富营销活动**：转盘抽奖、砸金蛋、优惠券、限时折扣等
 - ✅ **Uniapp 全端支持**：一套代码，多端发布（APP/小程序/H5）
 - ✅ **Webman 高性能后端**：高并发、低延迟，稳定支撑大流量场景
 - ✅ **全开源无加密**：无后门、无云验证，可商用、可二次开发

## 技术栈
- 前端：Uniapp
- 后端：Webman (PHP 高性能框架)
- 数据库：MySQL
- 支持：Android / iOS / H5 / 小程序

## 适用场景
- 短视频带货平台创业
- 电商直播、内容电商项目
- 分销代理、私域流量变现
- 二开学习、外包项目交付

## 版本定价
- 基础源码版：999 元（全套源码 + 数据库 + 部署指导）
- 高级售后版：1599 元（全套源码 + 6 个月更新 + 功能答疑）
- 成品部署版：2599 元（远程部署 + 环境配置 + 支付对接 + 直接上线）
- 定制开发：3500 元起

## 购买说明
1.  全套源码交付，无加密、可商用
2.  系统功能完整，可直接运行，适合二次开发
3.  提供基础技术支持与部署指导

## 联系方式
  QQ 

![输入图片说明](ScreenShot_2026-03-02_152533_332.png)
![输入图片说明](ScreenShot_2026-03-02_151747_525.png)
![输入图片说明](ScreenShot_2026-03-02_152533_332.png)
![输入图片说明](ScreenShot_2026-03-02_152619_381.png)
![输入图片说明](ScreenShot_2026-03-02_152634_861.png)
![输入图片说明](ScreenShot_2026-03-02_152929_885.png)


![输入图片说明](ScreenShot_2026-03-02_153321_673.png)
![输入图片说明](ScreenShot_2026-03-02_152236_538.png)
![输入图片说明](ScreenShot_2026-03-02_152117_698.png)
![输入图片说明](ScreenShot_2026-03-02_152021_186.png)
![输入图片说明](ScreenShot_2026-03-02_152053_442.png)
![输入图片说明](ScreenShot_2026-03-02_153154_218.png)
![输入图片说明](ScreenShot_2026-03-02_153239_657.png)
![输入图片说明](ScreenShot_2026-03-02_152258_090.png)
![输入图片说明](ScreenShot_2026-03-02_153908_826.png)
